...
class panel_file_hdr:
    ...
    def __init__(self, _f, data_types):
        ...
        # print(self.molecule_id)

        unpack_fmt = 47*'8s'
        size_header+=struct.calcsize(unpack_fmt)
        raw_bytes = _f.read(struct.calcsize(unpack_fmt))
        molecule_id_raw = struct.unpack(unpack_fmt, raw_bytes)
        self.molecule_id = [decode(id, encoding='utf-8') for id in molecule_id_raw]
        # print(self.molecule_id)
        # print(len(self.molecule_id))

        unpack_fmt = (64-47)*'8s'
        size_header+=struct.calcsize(unpack_fmt)
        raw_bytes = _f.read(struct.calcsize(unpack_fmt))
        rubbish = struct.unpack(unpack_fmt, raw_bytes)
        # print(rubbish)
        ...
        unpack_fmt = 47*data_types['float']
        raw_bytes = _f.read(struct.calcsize(unpack_fmt))
        size_header+=struct.calcsize(unpack_fmt)
        self.mol_col_dens = struct.unpack(unpack_fmt, raw_bytes)
        # print(self.mol_col_dens)
        # print(len(self.mol_col_dens))

        unpack_fmt = (64-47)*data_types['float']
        raw_bytes = _f.read(struct.calcsize(unpack_fmt))
        size_header+=struct.calcsize(unpack_fmt)
        rubbish_2 = struct.unpack(unpack_fmt, raw_bytes)
        # print(rubbish_2)
        ...
        unpack_fmt = '=8s8s6s6s6s46s' + data_types['junk']
        size_header+=struct.calcsize(unpack_fmt)
        raw_bytes = _f.read(struct.calcsize(unpack_fmt))
        data = struct.unpack(unpack_fmt, raw_bytes)
        # drop last junk word
        self.yid = [decode(point, encoding='utf-8') for point in data[0:-1]]
        # print("Size header: ", size_header)
    ...
    def __repr__(self):
        ...
        for n in range(len(self.mol_col_dens)):
            molecule_id_col_str = molecule_id_col_str + \
                f'    {self.molecule_id[n]:s} {self.mol_col_dens[n]:g}\n'
...
class panel_file:
    ...
    file. That is data1 will be radiance and data2 will be the
    tranmission. These attributes will be 1-D numpy ndarrays:
    ...
    def __init__(self, filename, do_load_data = False, 
        ...
        # print(junk_type, float_type, int_type)
        ...
        # print(self.hdr)
        print("Main header size: ", self._hdr_size)
    ...
    #     return
        ...
        # print("Single panel size: ", single_panel_size)
        # print("Double panel size: ", double_panel_size)
        ...
        # print("Checking panel for single")
        self._f.seek(self._hdr_size)
        panel_hdr = self._fread_panel_hdr()
        ...
        if not single_panel_valid:
            # print("Checking panel for double")
            self._f.seek(self._hdr_size)
            panel_hdr = self._fread_panel_hdr()
    ...
    def _check_length(self):
        ...
        while not at_last_panel and not at_EOF:
            ...
            # print("N points :", total_n_pts)
            n_pts_in_panel = panel_hdr[3]
            # n_pts_in_panel = 2400
            # print("N points in panel: ", n_pts_in_panel)
            if n_pts_in_panel > 0:
